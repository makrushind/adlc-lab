...
Внимание о приватности: для небольшого diff модель получает исходный diff, а
для большого — детерминированный ограниченный digest с метаданными изменённых
файлов, заголовками hunks и фрагментами добавленных строк. Полный diff остаётся
локальным для подготовки, lint и evidence. Найденные RAG-фрагменты по-прежнему
отправляются через Hugging Face gateway внешнему провайдеру модели. Redaction
trace скрывает данные только в локальном выводе и не скрывает отправленные модели
данные.
